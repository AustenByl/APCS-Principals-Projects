...
// This is Scramble.
// Remember to do the toupper thing
...
/**
 * Returns true if word, s, is found in grid.
 */
bool find(string s)
{
...
bool lookup(string s)
{
    ...
    // Problems:
    // don't know how to compare strings
    // Not sure how to set alphanumeric middle
    int start = 0;
    // Should check if s is equal to the middle word in the dictionary
    int end = WORDS - 1;
    while(start <= end)
    {
        int middle = (end + start) / 2;
        // Returns true and flags the word as found
        if (strcmp(s, dictionary.words[middle].letters) == 0)
        {
            if (dictionary.words[middle].found == true)
            {
                return false;
            }
            else
...
void scramble(void)
{
    // Itterates over the rows and collumns
    // Stores entire grid in gridTwo
    char gridTwo[DIMENSION][DIMENSION];
    for(int i = 0; i < DIMENSION; i++)
    {
        for(int x = 0; x < DIMENSION; x++)
        {
            gridTwo[i][x] = grid[DIMENSION - 1 - x][i];
        }
    }
    // Rewrites grid with gridTwo value positions
    for(int i = 0; i < DIMENSION; i++)
    {
        for(int x = 0; x < DIMENSION; x++)
        {
            grid[i][x] = gridTwo[i][x];
        }
    }
}
